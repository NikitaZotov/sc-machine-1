...
bool ScSet::Append(ScAddr const & elAddr, ScType const & arcType)
{
  if (!Has(elAddr, ScType::EdgeAccessConst))
    return m_context->CreateEdge(arcType, *this, elAddr).IsValid();
...
bool ScSet::Append(ScAddr const & elAddr, ScAddr const & attrAddr, ScType const & arcType)
{
  ...
  ScAddr const & arcAddr = m_context->CreateEdge(arcType, *this, elAddr);
  m_context->CreateEdge(ScType::EdgeAccessConstPosPerm, attrAddr, arcAddr);
...
{
  bool isFound = false;
  ScIterator3Ptr const iterator3 = m_context->Iterator3(*this, arcType, elAddr);
...
{
  return m_context->HelperCheckEdge(*this, elAddr, arcType);
}
...
ScSet const operator+(ScSet const & leftSet, ScSet const & rightSet)
{
  ...
  ScSet resultSet{context, context->CreateNode(ScType::NodeConst)};
