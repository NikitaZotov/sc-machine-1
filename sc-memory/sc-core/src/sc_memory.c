...
sc_bool sc_memory_check_read_local_and_global_permissions(sc_memory_context const * ctx, sc_addr element_addr)
{
  return _sc_memory_context_check_local_and_global_permissions(
      memory->context_manager, ctx, SC_CONTEXT_PERMISSIONS_READ, element_addr);
}
